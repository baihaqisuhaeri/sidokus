    ...
    <header class="bg-red-700 shadow-lg sticky top-0 z-50" x-data="{ open: false, userMenu: false }">
        ...
        <div :class="{'block': open, 'hidden': !open}" class="hidden md:hidden border-t border-red-600/50">
            <div class="px-4 py-3 space-y-1">
                ...
                <a href="{{ route('kegiatan.index') }}" class="nav-link flex items-center gap-2 px-3 py-2 rounded-lg text-sm">Kegiatan</a>
                <a href="{{ route('berkas.index') }}" class="nav-link flex items-center gap-2 px-3 py-2 rounded-lg text-sm">Berkas</a>
                @if(auth()->user()->isAdmin())
